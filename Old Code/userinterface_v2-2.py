...
# =============================================================================
    ...
    def make_layer_buttons(self, root):
        ...
        self.UnitModes_inverted= dict(map(reversed, self.UnitModes.items()))
        ...
        #MAKE THIS AUTOMATED BASED ON CSVS!!!
    ...
    def make_fill_list (self, root, shp):
        ...
        self.ZoneFields = dict(list(enumerate(shortfieldlist)))
        self.ZoneFields[0] = 'Nenhum'
        self.ZoneFields_inverted= dict(map(reversed, self.ZoneFields.items()))
        
        
        self.ZoneModes=dict(list(enumerate(longfieldlist)))
        self.ZoneModes[0] = 'Nenhum'
        self.ZoneModes_inverted= dict(map(reversed, self.ZoneModes.items()))
        ...
        for field in shp.fields:
            ...
            testfield = fieldnamelookup(fieldname)
            if testfield.fieldname != []:
                catdict[testfield.category].append(testfield.longname)
                
        
        self.zoneoptionvar = tk.StringVar()
        self.zoneoptionvar.set(list(self.ZoneModes_inverted.keys())[self.zones.color_setting.get()])
        
        # zoneoptionlist = tk.OptionMenu(root, self.zoneoptionvar, 
        #                   *list(self.ZoneModes_inverted.keys()), 
        #                   command=lambda _: self.replace_map_image())
        
     

        zoneoptionlist = tk.Menubutton(root, textvariable=self.zoneoptionvar, indicatoron=False)
        topMenu = tk.Menu(zoneoptionlist, tearoff=False)
        zoneoptionlist.configure(menu=topMenu)

        for key in sorted(catdict.keys()):
            menu = tk.Menu(topMenu)
            topMenu.add_cascade(label=key, menu=menu)
            for value in catdict[key]:
                menu.add_radiobutton(label=value, variable = self.zoneoptionvar, 
                                     value=value,
    ...
    def make_map_window(self, root):
        ...
        self.zoneoptionlist =  self.make_fill_list(self.frame_buttons, shps[0])
...
# =============================================================================
    ...
    def replace_map_image(self):
        ...
        if self.UnitModes[self.unitoptionvar.get()] != self.zones.setting.get():
            ...
            # print('new zone color setting: ' + str(self.zones.color_setting.get()))
            flag1 = 1
                                    
        else:
            self.zones.color_setting.set(self.ZoneModes_inverted[self.zoneoptionvar.get()])
        ...
        zonecolor = self.ZoneFields[self.zones.color_setting.get()]
        # zonecolortitle = self.zones.color_titles[self.zones.setting.get()][self.zones.color_setting.get()]
        zonecolortitle = self.ZoneModes[self.zones.color_setting.get()]
        ...
        if flag1 == 1:
            #Replace Zone Color Fill Option List
            self.zoneoptionlist.destroy()
            self.zoneoptionlist = self.make_fill_list(self.frame_buttons, shapename[0])
...
# =============================================================================
    def print_coords(self, event):
        ...
        for shaperec in self.zones.shps[self.zones.setting.get()].iterShapeRecords():
            ...
            if shapely.geometry.Point((geox, geoy)).within(shapely.geometry.shape(boundary)): # make a point and see if it's in the polygon
                ...
                for field in self.zones.shps[self.zones.setting.get()].fields:
                ...
                if self.ID != []:
                    ...
                    area_of_loss = 0;
                    for shaperec2 in self.overlays.shps[1].iterShapeRecords():
                    ...
                    self.list_zone.delete(0, tk.END)

                    for field in self.zones.shps[self.zones.setting.get()].fields:
                        fieldoutput = fieldnamelookup(field[0])
                        if fieldoutput.fieldname !=[]:
                            if fieldoutput.type == 'Other':
                                self.list_zone.insert(tk.END, fieldoutput.longname + ': ' + str(shaperec.record[fieldoutput.fieldname]))
                    ...
                    self.list_zone.insert(0, self.ID.longname +': ' + name)
                    if self.Action != []:
                        self.list_zone.insert(tk.END, self.Action.longname + ': ' + str(shaperec.record[self.Action.fieldname]))
                        self.list_zone.itemconfig(tk.END, {'bg':'yellow'})
...
# =============================================================================
    ...
    def zone_type(self,name):
        ...
        if name != [] and self.Action !=[]:
            ...
            windowtext = 'Definir ' + self.Action.longname + ' de ' + self.UnitModes_inverted[self.zones.setting.get()] 
            zone_window.title(windowtext)
            zone_window.geometry("+1200+750")
            lbl_text = tk.Label(zone_window, text=windowtext,font=('Arial',24))
            lbl_text.grid(column=1, row=0)
            
            zone_optionvar = tk.StringVar()
            zone_optionvar.set(self.Action.choices[0])
            zone_optionlist = tk.OptionMenu(zone_window, zone_optionvar,
                                           *self.Action.choices,
                                            command=lambda _: self.change_zone_type(name, zone_optionvar.get(),zone_window))
            zone_optionlist.grid(column=0, row=1)
    ...
    def change_zone_type(self,name,newtype, parent):
        ...
        self.list_zone.insert(tk.END, self.Action.longname + ': ' + str(newtype))
