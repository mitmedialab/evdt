...
import tkinter.font
import MapWindow_v3 as MapWindow
...
# =============================================================================
    ...
    # map_default = mapfilepaths[1]
    map_mangrove_extent = mapfilepaths[2]
    map_mangrove_health = mapfilepaths[3]
    
    zonenames = [[], 'POPDEN', 'EmpRate', 'AgriRate']
    # zone_default = zonenames[1]
    
    mangrovenames = [[], './FormatedShapefiles/Mangroves/mangrovelossshape.shp', './FormatedShapefiles/Mangroves/mangrovehealthshape.shp']
    shape_mangroveloss = mangrovenames[2]
    
    mangrovecolors = [[], [], 'mangroveHe']
    
    shapefilepaths = ['./FormatedShapefiles/Bairros_with_Population_Add.shp']
    ...
    def __init__(self, root):
        ...
        self.zone_default = self.zonenames[self.zonesetting.get()]
        self.mapsetting = tk.IntVar()
        self.mapsetting.set(1)
        self.map_default = self.mapfilepaths[self.mapsetting.get()]
        self.shapesetting= tk.IntVar()
        self.shapesetting.set(0)
        self.mangrovesetting = tk.IntVar()
        self.mangrovesetting.set(0)
        self.mangrove_default = self.mangrovenames[self.mangrovesetting.get()]
    ...
    def print_coords(self, event):
        ...
        self.list_zone.insert(tk.END, '2017 Employment: ' + str(emp))
        self.list_zone.insert(tk.END, '2017 Agricultural Employment: ' + str(agri))
    ...
    def zoomer(self, event, factor=None):
        ...
        self.configure(scrollregion=self.mapimg.bbox('all'))
...
# =============================================================================
    ...
    def make_layer_buttons(self, root):
        ...
        lbl_layers = tk.Label(frame_layer, text='Zone Fill', font=('Arial',24))
        ...
        ("Employment Rate", 2),
        ("Agricultural Employment Rate", 3)
        ]
        ...
        lbl_layers = tk.Label(frame_layer, text='Map Layer', font=('Arial',24))
        ...
        lbl_layers = tk.Label(frame_layer, text='Mangrove Layer', font=('Arial',24))
    ...
    def make_map_window(self, root):
        ...
        mapimg = MapWindow.Map(frame_map,[self.shape_bairros],background_image=self.map_default, color_range=[self.zone_default])
        # mapimg = MapWindow.Map(frame_map,[self.shape_bairros, self.shape_mangroveloss], background_image=self.map_default, color_range=[self.zone_default])
        
        # mapimg = MapWindow.Map(frame_map,[self.shape_mangroveloss], background_image=self.map_default)
...
# =============================================================================
    ...
    def replace_map_image(self):
        ...
        zonename = self.zonenames[self.zonesetting.get()]    
        mangrovename = self.mangrovenames[self.mangrovesetting.get()]
        ...
        if self.mapsetting.get() == 0:
            if self.zonesetting.get() == 0:
                self.mapimg = MapWindow.Map(self.frame_map,shapename)
                self.mapimg.bind("<Button-1>", self.print_coords)
            else:
                self.mapimg = MapWindow.Map(self.frame_map,shapename, color_range=[zonename])
                self.mapimg.bind("<Button-1>", self.print_coords)
        else:
            if self.zonesetting.get() == 0:
                self.mapimg = MapWindow.Map(self.frame_map,shapename, background_image=mapname)
                self.mapimg.bind("<Button-1>", self.print_coords)
            else:
                self.mapimg = MapWindow.Map(self.frame_map,shapename, background_image=mapname, color_range=[zonename])
                self.mapimg.bind("<Button-1>", self.print_coords)
                
        if self.mangrovesetting.get() != 0:
            self.mapimg.addshapes(mangrovename, outline='black', color_range=self.mangrovecolors[self.mangrovesetting.get()])
